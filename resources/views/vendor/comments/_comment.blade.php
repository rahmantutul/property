...
<div id="comment-{{ $comment->getKey() }}" class="media  bg-light-success p-1">
    <img class="mr-1" height="70" width="70" style="border-radius: 50%; border: 1px solid green;"  src="{{ asset('images/defaultUser.png') }}" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
    <div class="media-body">
        <h5 class="mt-0">{{ $comment->commenter->email ?? $comment->guest_name }} <small class="text-muted">- {{ $comment->created_at->diffForHumans() }}</small></h5>
        <div style=" white-space: pre-wrap; color:#000;">{!! $markdown->line($comment->comment) !!}</div>

        <div>
            @can('reply-to-comment', $comment)
                <button data-toggle="modal" data-target="#reply-modal-{{ $comment->getKey() }}" class="btn btn-sm btn-success text-uppercase">@lang('comments::comments.reply')</button>
            @endcan
            ...
            @can('delete-comment', $comment)
                <a href="{{ route('comments.destroy', $comment->getKey()) }}" onclick="event.preventDefault();document.getElementById('comment-delete-form-{{ $comment->getKey() }}').submit();" class="btn btn-sm btn-link btn-danger text-uppercase">@lang('comments::comments.delete')</a>
