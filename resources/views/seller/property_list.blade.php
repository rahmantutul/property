...
<div class="row mb-1">
    ...
    <a class="btn btn-primary btn-round btn-sm " href="{{route('seller.property.create')}}">Add New Property</a>
    </div>
</div>
                        ...
                        @foreach($dataList as $key=>$dataInfo)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{route('seller.property.feature.change',['dataId'=>$dataInfo->id,'is_featured'=>($dataInfo->is_featured==0)?1:0])}}" class="btn {{getFeatureClass($dataInfo->is_featured)}} btn-sm btn-icon btn_status_change" title="Change Feature Status">
                                ...
                                <td>
                                    ...
                                    @if (isset(request()->is_featured))
                                        <a href="{{route('seller.property.feature.change',['dataId'=>$dataInfo->id,'is_featured'=>($dataInfo->is_featured==0)?1:0])}}" class="btn btn-danger btn-sm btn-icon btn_status_change" title="Remove">
                                            @if ($dataInfo->is_featured==0)
                                             <i data-feather='check'></i>
                                            @else
                                             <i data-feather='x'></i>
                                    ...
                                    @else
                                    <a href="{{route('seller.property.status.change',['dataId'=>$dataInfo->id,'status'=>($dataInfo->status==1)?2:1])}}" class="btn btn-sm btn-icon {{getStatusChangeBtn($dataInfo->status)}} btn_status_change" title="Change Status">
                                        {!!getStatusChangeIcon($dataInfo->status)!!}
                                    </a>
                                    <a href="{{route('seller.property.edit',['dataId'=>$dataInfo->id])}}" class="btn btn-warning btn-sm btn-icon " title="Edit">
                                        <i data-feather='edit'></i>
                                    </a>
                                    <a href="{{route('seller.property.delete',['dataId'=>$dataInfo->id])}}" class="btn btn-danger btn-sm btn-icon {{getStatusChangeBtn($dataInfo->status)}} delete" title="Delete">
