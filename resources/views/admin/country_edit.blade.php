<div class="modal-dialog modal-dialog-top modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="exampleModalLabel">Country Update</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="background-color: red!important;">
                <span aria-hidden="true" style="color:white;">&times;</span>
            </button>
        </div>
        <div class="modal-body">
            <form class="row" id="ajax_form" action="{{route('admin.country.update')}}" method="POST">
                @csrf
                <input type="hidden" name="dataId" value="{{$dataInfo->id}}">
                <div class="col-12 form-group">
                    <strong>Country Name</strong>
                    <input type="text" name="name" placeholder="Country Name" value="{{ $dataInfo->name }}"  class="form-control" required>
