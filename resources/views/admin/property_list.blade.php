                    ...
                    <table class="table">
                        ...
                        @foreach($dataList as $key=>$dataInfo)
                            <tr>
                                ...
                                <td>
                                    @if (isset(request()->is_featured))
                                        <a href="{{route('admin.property.feature.change',['dataId'=>$dataInfo->id,'is_featured'=>($dataInfo->is_featured==1)?2:0])}}" class="btn btn-success btn-sm btn-icon btn_status_change" title="Approve">
                                             <i data-feather='check'></i>
                                        </a>
                                        <a href="{{route('admin.property.feature.change',['dataId'=>$dataInfo->id,'is_featured'=>($dataInfo->is_featured==1)?0:1])}}" class="btn btn-danger btn-sm btn-icon btn_status_change" title="Approve">
                                             <i data-feather='x'></i>
                                        </a>
                                    @else
                                    <a href="{{route('admin.property.status.change',['dataId'=>$dataInfo->id,'status'=>($dataInfo->status==1)?2:1])}}" class="btn btn-sm btn-icon {{getStatusChangeBtn($dataInfo->status)}} btn_status_change" title="Change Status">
                                        {!!getStatusChangeIcon($dataInfo->status)!!}
                                    </a>
                                    <a href="{{route('admin.property.edit',['dataId'=>$dataInfo->id])}}" class="btn btn-warning btn-sm btn-icon " title="Edit">
                                        <i data-feather='edit'></i>
                                    </a>
                                    <a href="{{route('admin.property.delete',['dataId'=>$dataInfo->id])}}" class="btn btn-danger btn-sm btn-icon {{getStatusChangeBtn($dataInfo->status)}} delete" title="Delete">
